// CODE FOR READING HISTORICAL WEATHER DATA FROM A COSM FEED WE SET UP
// By KJ Chabra, Wesley Chan, Felipe Scarpelli, Maria Enache

//Servo libraries
#include <AFMotor.h>
#include <Servo.h> 

// delcare servos
Servo servoDisc;
Servo servoArm;

// declare moving variables
int rotDisc = 360;
float rotArm;
int variation = 0; // variable for creating patterns according to weather variation

//Cosm, https and ehternet library for arduino
#include <SPI.h>
#include <Ethernet.h>
#include <HttpClient.h>
#include <Cosm.h>

// MAC address for our Ethernet shield
byte mac[] = { 
  0x00, 0xA1, 0xB1, 0xC1, 0xD1, 0x03 };

// Cosm key to upload data
char cosmKey[] = "LV0PdJ76vmRUPe8mmHQHHjv20NySAKx1VjF2aTFOemp2OD0g";

// Define the string for our datastream ID
char temperatureId[] = "Historical";

//Array for the data feed coming in
CosmDatastream datastreams[] = {
  CosmDatastream(temperatureId, strlen(temperatureId), DATASTREAM_FLOAT),
};

// Finally, wrap the datastreams into a feed
CosmFeed feed(90162, datastreams, 1 /* number of datastreams */);

//call out the ehternet shield variable
EthernetClient client;

//call out cosms variable
CosmClient cosmclient(client);

//declare setup
void setup(){
  
  // set up servors on arduino
  servoDisc.attach(6);
  servoArm.attach(5);

  // initiate serial connection
  Serial.begin(9600);

  // set starting point for compass
  servoArm.write(79);

  //print out messages for setup
  Serial.println("Reading from Cosm example");
  Serial.println("hello");
  
  //if no feed is availble or shield isn't able to connect print out error
  int FEED = cosmclient.get(feed, cosmKey); 
  while (Ethernet.begin(mac) != 1)
  {
    Serial.println("Error getting IP address via DHCP, trying again...");
  }
}

void loop() {
   //variation variable to alter the arm motor
  if (variation == 0) {
    variation = 1;
  }
  else {
    variation = 0;
  }
  
  //connect to cosm with a key and available feeds from the feed ID
  int ret = cosmclient.get(feed, cosmKey);
  
  //variable to control the motors rotation
  rotArm = feed[0].getFloat();
  
  //final variable to control the motor arm
  float rotFinal = rotArm + variation;
  
  //print out variation variable in serial monitor
  Serial.println(variation);
  
  //print out rotArm variable in serial monitor
  Serial.println(rotArm);
  
  //get the base of the machine spinning
  servoDisc.write(rotDisc);
  
  //get the arm of servo moving with the feed coming in + the value of variation
  servoArm.write(rotFinal);  

}
